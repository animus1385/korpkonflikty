<template>
    <section class="corp-servies" id="corp-servies"
        v-if="props.data.name == 'CorpServiesCommon' || props?.data.name == 'CorpServiesCustom' && props.data.fields">
        <div class="corp-servies__container container">
            <div class="corp-servies__top">
                ...
                <div class="corp-servies__nav-btns" v-if="$viewport.isGreaterOrEquals('tablet')">
                    <UButton class="corp-servies__nav-btn" @click="swiper.prev()" variant="ghost"
                        icon="custom-icons:arrow-slide-prev">
                    </UButton>
                    <UButton class="corp-servies__nav-btn" @click="swiper.next()" variant="ghost"
                        icon="custom-icons:arrow-slide-next">
                    </UButton>
                </div>
            </div>
            <swiper-container ref="swiperRef" :space-between="50" :breakpoints="{
                320: {
                    slidesPerView: 1,
                },
                1024: {
                ...
                <swiper-slide class="corp-servies__elem-swiper"
                    v-for="elem in props?.data?.fields?.corpServiesList?.nodes" :key="elem.id">
                    <NuxtLink class="corp-servies__elem" :to="elem.uri">
                        <NuxtImg v-if='elem?.featuredImage?.node?.sourceUrl' class="corp-servies__img"
                            :src="elem.featuredImage.node.sourceUrl" :alt="elem.title">
                        </NuxtImg>
                        <div class="corp-servies__elem-info">
                            ...
                            <p class="corp-servies__elem-descr"
                                v-if="elem?.pageBuilder?.flexible[0]?.heroCustom?.subtitle">
